...
>这个repository是我在暑期集训期间建立的，所有暑假里写的题目都在这里
[ACM-ICPC](https://github.com/Ranc10m/ACM-ICPC)

在暑期集训以及集训完的假期中，我完成了 _7.22vjudge_
专题训练的大部分题目，和在 _51nod_ 算法网站上做了*168*的题目，
和对多项式的一些知识进行了系统地学习。下面就这些做过题目做一下整理和相应题目简单的报告.
以及自己对多项式相关算法的整理。


* ### 51nod
这个是我在51nod上所做的题目：[51nod题目](https://www.51nod.com/onlineJudge/user.html#!userId=9369)  
51nod将题目根据难度进行了分类，从基础题，一级题目到九级题目，和难题，级数越高，难度越大，前面一二级
的题目比较基础，三级级及以后有很多值得思考的题目。除了难题，其他级别或多或少做了一些题目，
基础题(42/42),1级(20/20),二级(27/27),三级(40/40),四级(15/72),5级(9/98),6级(8/84),7级(3/79),
8级(2/56),9级(2/23),难题(0/6),难题是一些有趣的问题,需要仔细正确地思考，难度太大，没有解决任何一道题目。  
接下来按照级别对题目做一些整理
...
先用Kruskal 算出最大边的最小值,再用一下Kruskal算法算出最大生成树
...
非常麻烦的一道数位dp。利用卷积公式的的一些性质入手，降低位数，然后迭代。 
...
考虑建立一个有n个节点的一个图，x到f(x)建立一条边,找到每个联通分量中环的节点数，那么k需要满足能被每个环的大小整除，
并且k必须要大于每个环支链节点的数量，那么对每个节点dfs一下就可以了。

* 1086 背包问题 V2
>有N种物品，每种物品的数量为C1，C2......Cn。从中任选若干件放在容量为W的背包里，每种物品的体积为W1，W2......Wn（Wi为整数），与之相对应的价值为P1,P2......Pn（Pi为整数）。
求背包能够容纳的最大价值。

转换为01背包来做，将c分解成c=2^0+2^1+...+2^m+r,其中r<2^m
...
设三个数分别为x,y,z并且 x>y>z ,那么可以在二进制位中调整x,z的值，使x,y,z的值相对接近如  
110000   
101111  
011111  
所以直接找到大于l的满足二进制形式是110000的数，如果小于等于r,则可以构造异或和为0、

* 1537 分解
> 问(1+sqrt(2)) ^n  能否分解成 sqrt(m) +sqrt(m-1)的形式 
如果可以 输出 m%1e9+7 否则 输出no

把m反解出来，利用二次剩余来做，至于为什么一定有解，可以通过佩尔方程的一些定理来证明

* 1586 约数和
>有三个下标从1到n的数组a、b、c。a数组初始全为0。  
b[i]=sigma(a[j]) (j|i)  
c[i]=sigma(b[j]) (j|i)  
需要进行下列操作：
1 x y :将a[x]加上y
...
则若a[i]的增量为x，c[j]会受到影响当且仅当i|j，且c[j]的增量为x*f[j/i]（f[x]表示x的约数个数，
相当于多少个上式中的 k1∗k2=i/j ,因为修改位置x为随机，所以每次暴力枚举x的倍数作为要修改的c数组下标即可
f数组可以O（nlogn）暴力算出，也可以线性筛O（n）算出。时间复杂度O（mlogn）

* 1451 合法三角形
>有n个不同的点，问有多少组三元组能构成面积非0的三角形。

枚举所有直线，用所有三元组的总数减去所有够不成三角形的数量
...
水仙花数一共只有89个，可以先把表给打出来，然后二分查找

* 1029 大数除法
>给出2个大整数A,B，计算A / B和A Mod B的结果。

大数除法效率高的前提是，需要有n*log(n)的大数乘法运算。大概思路就是，将大数除法转为大数乘法。将A/B转为A * (1/B)。
1/B可以不用除法来算，而是用（+-*）来算，方法就是牛顿迭代。由于牛顿迭代二阶收敛，哪怕是大数，也只要log(n)次，就可以达到精度的要求。

* 1165 整边直角三角形的数量
>直角三角形，三条边的长度都是整数。给出周长N，求符合条件的三角形数量。
例如：N = 120，共有3种不同的满足条件的直角3角行。分别是：{20,48,52}, {24,45,51}, {30,40,50}。

假设三边长为a,b,c,且a<b<c,a^2+b^2=c^2.于是a+b+sqrt(a^2+b^2)=n,化简得到b=n*(n-2*a)/(2*(n-a))  
令t=n-a,则，b=n-n*n/2*t, n/sqrt(2)<t<n

* 1228 序列求和
>T(n) = n^k，S(n) = T(1) + T(2) + ...... T(n)。给出n和k，求S(n)。例如k = 2，n = 5，S(n) = 1^2 + 2^2 + 3^2 + 4^2 + 5^2 = 55。
由于结果很大，输出S(n) Mod 1000000007的结果即可。

等幂和问题，用快速数论变换预处理伯努利数，因为模数不具有特殊性，所以考虑采用三个满足要求的素数p1,p2,p3,
只需要p1p2p3>n*(mod-1)^2即可，复杂度O(nlogn),常数有些大。

* 1498 距离平方和的最大值
>下面要求从平面中选取n个格点，使得他们之间的距离平方的和要最大。设第i个点是 (xi,yi) ，这些点要满足 xi^2+yi^2 ≤ r^2 。
这儿的距离说的是两个点之间的直线距离。

1.距离平方和可以转化为n^2倍的x方向方差+y方向方差  
2.在方差最大化的方案中，其所有的x与y必然都最大化远离平均值（否则可以构造更优解）。  
将接近边缘的点存下来，也即到原点的距离大于r-1，小于r,然dfs
...
令f[n]为长度为n的答案
则： f[0]=0 f[n]=i!-sigma(f[j]*(i-j)!)  0<=j<i
我们将其变形 Sigma(f[j]*(i-j)!)=i!   j=0~i
令G(x)=sigma(i!*x^i),F(x)=sigma(f[i]*x^i)
F(x)*G(x)=G(x)-1 F(x)=(G(x)-1)/G(x)=1-1/G(x)
多项式求逆fft
...
由于 n 非常大，所以 n 被表示成了 m 个正整数 x1,x2,⋯,xm 的乘积，即 n=∏(i=1->m)xi 。
由于 m 也非常大，所以对于 xi(1≤i<m) 有 xi+1=((a⋅xi+b)modc)+1 的关系。
上述的 m,x1,a,b,c 是给定的正整数。

利用f(n)和g(n)都是积性函数，化简后得到答案ans=n*div_num(n),div_num(n)表示n的约数个数。
首先找到xi的循环节，然后进行因数分解，可以降序枚举xi,分解质因数进行计算
...
固定k后对sg(n)进行打表，可以发现每个数字第一次出现的位置很有规律，即每隔几个数字就出现一个已有的数字。
删去每个数字的第一次出现位置之后，将剩下的数字再排成一列，和原来的表是同构的，可以猜测sg(n)的递归解是sg(1)=0, sg(ak+1)=sg(a), sg(ak+b)=a(k-1)+b-1(1<b<=k,注意b的定义)。
本题为n个游戏的和，所以对应的sg值为sg(a1) xor sg(a2) xor ... xor sg(an)。
如果sg值为0，则后手必胜。
...
这道题用的是GarsiaWachs算法，复杂度是O(n^2),GarsiaWachs算法是从第一个石堆开始找符合stone[k-1]<stone[k+1]的石堆k，然后合并k-1与k堆，再向前找j堆石子，
满足stone[j]>stone[k]+stone[k-1],插入到石堆j的后面。然后重新寻找。

* 1059 N的阶乘 V2
>输入N，求N的阶乘的准确值。
数据规模：1<=n<=100000

然后用分治，把这个阶乘从中间分成两半，这样不断分，总复杂度为O(n(logn)^2)，大数乘法用FFT。

* 1348 乘积之和
>给出由N个正整数组成的数组A，有Q次查询，每个查询包含一个整数K，从数组A中任选K个（K <= N）把他们乘在一起得到一个乘积。求所有不同的方案得到的乘积之和，
由于结果巨大，输出Mod 100003的结果即可。例如：1 2 3，从中任选1个共3种方法，{1} {2} {3}，和为6。从中任选2个共3种方法，{1 2} {1 3} {2 3}，和为2 + 3 + 6 = 11。

从中间分成两半，分别算出两边的生成函数，然后再乘起来。

* ##### 八级算法题(2/56)

* 1172 Partial Sums V2
>给出一个数组A，经过一次处理，生成一个数组S，数组S中的每个值相当于数组A的累加，比如：A = {1 3 5 6} => S = {1 4 9 15}。如果对生成的数组S再进行一次累加操作，
{1 4 9 15} => {1 5 14 29}，现在给出数组A，问进行K次操作后的结果。(输出结果 Mod 10^9 + 7)

前面写过v1版本，题目是一样的只是这个题的数据更强一点。

* 1667 概率好题
>甲乙进行比赛。
他们各有k1,k2个集合[Li,Ri],每次随机从他们拥有的每个集合中都取出一个数，
S1=sigma甲取出的数，S2同理,若S1>S2甲胜，若S1=S2平局，否则乙胜。
分别求出甲胜、平局、乙胜的概率。
（显然这个概率是有理数，记为p/q，则输出答案为（p/q）%(1e9+7)）（逆元）

考虑将甲的区间[L,R]拆成[-inf,L-1]和[-inf,R]，将乙的区间[L,R]拆成[R+1,inf]和[L,inf]，
再用容斥，考虑相对差，对于赢，用k1+k2+1个数(>=0)去填补相对差，
对于平，用k1+k2个数(>=0)去填补相对差，每次计算贡献用dfs即可
...
* 1728 不动点
>不动点是一个“由函数定义域映射到定义域本身”的函数的恒定元素。也就是说，一个元素 c 是一个函数 f 的不动点，当且仅当 f(c)=c 。这也意味着 f(f(⋯f(c)⋯))=fn(c)=c ，这是计算递归函数 f 的一个重要的结束条件。
一个满足定义域和值域相同的函数可能没有太多的不动点，但如果函数 f 的定义域元素可数，并且对于定义域内任意的一个元素 c ，经过函数 f 有限次的映射后总会有 f(c′)=c′ ，其中 f(f(⋯f(c)⋯))=fn(c)=c′ ，
 n 为非负整数，那么这个函数 f 一定是有不动点的，这样的函数被称为可解的递归函数。
有时候需要考虑的是：可解的递归函数与定义域的元素个数 s 之间的关系。
例如，定义域是 {a,b} 时，可能的函数有 4 种，但是可解的递归函数只有 3 种，不可解的那个情况是 f(a)=b,f(b)=a 。
然而，善于思考的Noder已经发现了它的性质：可解的递归函数恰好有 (s+1)s−1 种。
但是，如果再给出一个正整数 k ，求解“对于任意属于定义域的元素 c ，存在非负整数 n<k 满足 f(f(⋯f(c)⋯))=fn(c)=c′ 且 f(c′)=c′ ”的函数种数，你还会算吗？
注意 c 可以是定义域内的任意一个元素，而 f0 是恒等映射，对于任意属于定义域的元素 c ，满足 f0(c)=c 。
这个答案可能很大，你只需要给出它对 985661441 取模的值即可。

因为模数特殊，可以考虑求生成函数，生成函数是e^(x*e^x),利用多项式求指数和求对数来预处理答案


* ### vjudge

由于vjudge的站点的打不开，所以这部分的解题报告就没有写，只是做一个简单的归纳，等vjudge站点好了以后再补上
* ##### 7.22
这场是完成题目最多的一场，完成了17道题目  
涉及到的一些题目类型有概率dp，矩阵快速幂，二分图匹配，线段数

* ##### 7.17
这场只完成了6个题目，都是codeforces的题目  


* ### 多项式

多项式方面，我从快速傅里叶变换入手，相继学习了，多项式乘法，多项式求逆，多项式除法，多项式求逆，
多项式对数，多项式求指数，以及更一般的针对多项式的牛顿迭代法,多项式的相关算法多用于生成函数的计算。
因为浮点型的快速傅里叶变换和整数的快速傅里叶变换，本质上是一样的，所以下面的代码都是快速数论变换的。
...
实际上利用的是Reverse(A(x))=Reverse(D(x))Reverse(B(x))(modx^(n−m+1)),
Reverse()进行是将多项式的系数进行前后转置
...
下面几个关于多项式的算法原理都是牛顿迭代，为了提高效率，我都是用迭代的方法写的
